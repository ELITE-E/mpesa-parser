...
from src.engine import calculate_transfer_efficiency, process_engine
...
def test_engine_handles_first_row_orphan_fee():
    ...
    orphan_df = pd.DataFrame(
        [
            {
                "Receipt No.": "Z1",
                "Details": "M-Pesa Statement Fee Rule",
                "Withdrawn": 10.0,
            }
        ]
    )
    ...
    assert (
        processed.loc[0, "Total Cost"] == 10.0
    )  # Stays 10 because there is no parent row at index -1
